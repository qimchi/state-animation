...
export const DopeCheckPop = React.memo<DopeCheckPopProps>(({
  visible,
  size = 72,
  color = '#31CC66',
  onComplete,
  soundPath = '/images/pop2.mp3',
  playSound = true
}) => {
  ...
  // Handle animation start to trigger sound (most reliable)
  const handleAnimationStart = () => {
    if (playSound && !hasPlayedRef.current) {
      const playAudio = () => {
        if (audioRef.current) {
          try {
            ...
            audioRef.current.play()
              ...
              .catch((error) => {
                ...
                // Try creating new audio instance
                try {
  ...
  // Also play when visible prop changes (backup)
  useEffect(() => {
    if (visible && playSound && !hasPlayedRef.current) {
      // Use animation start as primary trigger, this is just backup
  ...
  return (
    ...
    >
      <svg width={size} height={size} viewBox="0 0 143 143" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          fillRule="evenodd"
          clipRule="evenodd"
...
DopeCheckPop.displayName = 'DopeCheckPop';
